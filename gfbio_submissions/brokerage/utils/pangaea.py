...
from gfbio_submissions.brokerage.configuration.settings import CSV_WRITER_QUOTING, SEPARATOR, SUBMISSION_DELAY
...
def request_pangaea_login_token(resource_credential):
    ...
    body = textwrap.dedent(
        """<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:urn="urn:java:de.pangaea.login.PanLogin">
  <soapenv:Header/>
  <soapenv:Body>
    <urn:login>
...
def prepare_pangaea_issue_content(site_configuration, submission):
    data = {
        ...
        "customfield_10004": submission.data.get("requirements", {}).get("title", ""),
        "issuetype": {
...
def pull_pangaea_dois(submission, jira_client):
    ...
    for p in references:
        ...
        if doi:
            ...
            from gfbio_submissions.brokerage.tasks import add_pangaea_doi_task
