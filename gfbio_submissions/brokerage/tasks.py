...
@celery.task(
    base=SubmissionTask,
    bind=True,
    name='tasks.notify_user_reporter_changed_task',
    autoretry_for=(TransferServerError,
                   TransferClientError
                   ),
    retry_kwargs={'max_retries': SUBMISSION_MAX_RETRIES},
...
def notify_user_reporter_changed_task(self, prev=None, submission_id=None):
    logger.info(
        'tasks.py | notify_user_reporter_changed_task | submission_id={0}'.format(
            submission_id))
    ...
    if site_config and site_config.helpdesk_server:
        ...
        if reference:
            comment = get_jira_comment_template(
                template_name="NOTIFY_REPORTER_CHANGED",
                task_name="notify_user_reporter_changed_task")
            if not comment:
                return TaskProgressReport.CANCELLED
