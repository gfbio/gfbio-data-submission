...
GFBIO_HELPDESK_TICKET = getattr(settings, "GFBIO_HELPDESK_TICKET", "0")

PANGAEA_JIRA_TICKET = getattr(settings, "PANGAEA_JIRA_TICKET", "1")
...
APPROVAL_EMAIL_MESSAGE_TEMPLATE = getattr(
    settings, "APPROVAL_EMAIL_MESSAGE_TEMPLATE", "Please review submission {0}.\n{1}"
)

BASE_HOST_NAME = getattr(settings, "BASE_HOST_NAME", "submission.gfbio.org")
...
CSV_WRITER_QUOTING = getattr(settings, "CSV_WRITER_QUOTING", QUOTE_NONNUMERIC)

CSV_READER_QUOTING = getattr(settings, "CSV_READER_QUOTING", QUOTE_NONE)
...
DEFAULT_ENA_CENTER_NAME = getattr(settings, "DEFAULT_ENA_CENTER_NAME", "")

ENA_PANGAEA = getattr(settings, "ENA_PANGAEA", "ENA_PANGAEA")

GENERIC = getattr(settings, "GENERIC", "GENERIC")

ATAX = getattr(settings, "ATAX", "ATAX")

# https://helpdesk.gfbio.org/secure/ViewProfile.jspa?name=brokeragent
JIRA_FALLBACK_USERNAME = getattr(settings, "JIRA_FALLBACK_USERNAME", "brokeragent")
...
JIRA_COMMENT_SUB_URL = getattr(settings, "JIRA_COMMENT_SUB_URL", "comment")

JIRA_ATTACHMENT_URL = getattr(settings, "JIRA_ATTACHMENT_URL", "/rest/api/2/attachment")

JIRA_ATTACHMENT_SUB_URL = getattr(settings, "JIRA_ATTACHMENT_SUB_URL", "attachments")
...
GFBIO_DATACENTER_USER_MAPPINGS = getattr(
    ...
    "GFBIO_DATACENTER_USER_MAPPINGS",
    {
        ...
        "e!DAL-PGP – Plant Genomics and Phenomics Research Data Repository": "edal",
        ...
        "LIB – Leibniz Institute for the Analysis of Biodiversity Change": "lib",
...
GFBIO_REQUEST_TYPE_MAPPINGS = getattr(
    ...
    {"ENA – European Nucleotide Archive": "molecular", "default": "generic"},
)
...
SUBMISSION_UPLOAD_MAX_RETRIES = getattr(settings, "SUBMISSION_UPLOAD_MAX_RETRIES", 2)
SEPARATOR = getattr(settings, "SEPARATOR", ".")
...
SUBMISSION_MAX_RETRIES = getattr(settings, "SUBMISSION_MAX_RETRIES", 2)
...
STATIC_ENA_REQUIREMENTS_LOCATION = getattr(
    settings, "STATIC_ENA_REQUIREMENTS_LOCATION", "schemas/ena_requirements.json"
)
