...
from ...tasks.jira_tasks.add_accession_link_to_submission_issue import add_accession_link_to_submission_issue_task
...
from ...tasks.jira_tasks.add_posted_comment_to_issue import add_posted_comment_to_issue_task
...
class TestGFBioHelpDeskTasks(TestHelpDeskTasksBase):
    ...
    def test_tpr_add_pangaea_link_server_error(self):
        ...
        responses.add(
            responses.GET,
            "{0}/rest/api/2/field".format(self.default_site_config.helpdesk_server.url),
            status=200,
        )
        url = "{0}{1}/{2}/{3}".format(
    ...
    def test_create_submission_issue_task_success(self):
        ...
        responses.add(
            responses.POST,
            "{0}{1}".format(site_config.helpdesk_server.url, JIRA_ISSUE_URL),
            json=self.issue_json,
            status=200,
        )
        responses.add(
            responses.GET,
            "{0}/rest/api/2/issue/SAND-1661".format(site_config.helpdesk_server.url),
            json=self.issue_json,
        )
    ...
    def test_add_posted_comment_to_issue_task_success(self):
        ...
        result = add_posted_comment_to_issue_task.apply_async(
            kwargs={"submission_id": submission.id, "comment": "a comment"}
        )
        self.assertTrue(result.successful())
        self.assertTrue(result.get())
