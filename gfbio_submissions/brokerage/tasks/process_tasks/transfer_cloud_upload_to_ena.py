...
def ensure_folder_with_keep(path):
    ...
    if not os.path.exists(keep_file):
        logger.info(
            f"tasks.py | transfer_cloud_upload_to_ena_task | ensure_folder_with_keep | "
...
def perform_ascp_file_transfer(
            ...
            if submission_cloud_upload.status != SubmissionCloudUpload.STATUS_TRANSFER_FAILED:
                ...
                submission_cloud_upload.log_change(
                    [
                        {
                            "changed": {
                                "fields": [f"status changed to {submission_cloud_upload.status} due to {stderr_str}"]
                            }
                        }
                    ],
                    user_id,
                ...
                submission_cloud_upload.log_change(
                    [
                        {
                            "changed": {
                                "fields": [f"status kept at {submission_cloud_upload.status} due to {stderr_str}"]
                            }
                        }
                    ],
                    user_id,
        ...
        if submission_cloud_upload.status != SubmissionCloudUpload.STATUS_TRANSFER_FAILED:
            ...
            submission_cloud_upload.log_change(
                [
                    {
                        "changed": {
                            "fields": [f"status changed to {submission_cloud_upload.status} due to {details['error']}"]
                        }
                    }
                ],
                user_id,
            ...
            submission_cloud_upload.log_change(
                [
                    {
                        "changed": {
                            "fields": [f"status kept at {submission_cloud_upload.status} due to {details['error']}"]
                        }
                    }
                ],
                user_id,
        ...
        if calculated_md5sum == submission_cloud_upload.file_upload.md5:
            ...
            submission_cloud_upload.log_change(
                [
                    {
                        "changed": {
                            "fields": [f"status changed to {submission_cloud_upload.status} ({checksum_message})"]
                        }
                    }
                ],
                admin_user.pk,
...
def transfer_cloud_upload_to_ena_task(
    self, previous_result=None, submission_cloud_upload_id=None, submission_id=None, user_id=None
):
    ...
    if not os.path.exists(file_path):
        ...
        submission_cloud_upload.log_change(
            [
                {
                    "changed": {
                        "fields": [f"status changed to {submission_cloud_upload.status} (File not found in bucket)"]
                    }
                }
            ],
            user_id,
