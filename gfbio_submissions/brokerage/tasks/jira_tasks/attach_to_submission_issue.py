...
def attach_cloud_upload_to_submission_issue_task(
    ...
    # TODO: to method .....
    logger.info(
        msg="{0}. submission_id={1} | submission_upload_id={2}"
            "".format(self.name, submission_id, submission_upload_id)
    )
    ...
    if submission == TaskProgressReport.CANCELLED:
        logger.info(
            msg="attach_cloud_upload_to_submission_issue_task no Submission"
                " found. return {2}. | submission_id={0} | submission_upload_id={1}"
                "".format(submission_id, submission_upload_id, TaskProgressReport.CANCELLED)
        )
        return TaskProgressReport.CANCELLED
    ...
    if reference:
        ...
        submission_upload = submission.submissioncloudupload_set.filter(attach_to_ticket=True).filter(
            metadata=True).filter(pk=submission_upload_id).first()

        # TODO: to method, upload as arguement, then interface is the same
        logger.info(msg=f"{self.name} | submission_upload={submission_upload}")
        if submission_upload:
            do_attach = False
            if submission_upload.attachment_id is None:
                do_attach = True
            ...
            logger.info(msg=f  "attach_to_submission_issue_task | do_attach={0} | return {1}".format(do_attach, True))
