...
class GFBioAuthenticationBackend(OIDCAuthenticationBackend):
    ...
    def get_username(self, claims):
        ...
        logger.info("GFBioAuthenticationBackend | get_username | username={0}  | " "".format(username))
        return unicodedata.normalize("NFKC", username)[:150]

    def create_user(self, claims):
        logger.info("GFBioAuthenticationBackend | create_user | claims={0}  | " "".format(claims))
        user = super(GFBioAuthenticationBackend, self).create_user(claims)

        user.first_name = claims.get("given_name", "")
        user.last_name = claims.get("family_name", "")
        user.name = "{0} {1}".format(claims.get("given_name", ""), claims.get("family_name", "")).strip()
        user.email = claims.get("email", "")

        # user.external_user_id = claims.get('goe_id', '')
        user.update_or_create_external_user_id(external_id=claims.get("goe_id", ""), provider="goe_id")
    ...
    def update_user(self, user, claims):
        logger.info("GFBioAuthenticationBackend | update_user | claims={0}  | user={1}" "".format(claims, user))
        user.first_name = claims.get("given_name", "")
        user.last_name = claims.get("family_name", "")
        user.name = "{0} {1}".format(claims.get("given_name", ""), claims.get("family_name", "")).strip()
        user.email = claims.get("email", "")
