...
from gfbio_submissions.brokerage.models import AdditionalReference, Submission
...
class JiraHookRequestSerializer(serializers.Serializer):
    ...
    def send_mail_to_admins(
        ...
        submission_msg = (
            f"Submission ID: {self.broker_submission_id}" if add_submission_id else ""
        )
    ...
    def save(self):
        try:
            embargo_date = arrow.get(
                self._data_get(
                    self.validated_data, ["issue", "fields", "customfield_10200"]
                )
            )
        ...
        submission_id = self._data_get(
            self.validated_data, ["issue", "fields", "customfield_10303"]
        )
        try:
    ...
    def update_submission_user(self):
        ...
        reporter_name = self._data_get(
            self.validated_data, ["issue", "fields", "reporter", "name"]
        )
        if reporter_name:
            new_reporter["jira_user_name"] = reporter_name

        reporter_email = self._data_get(
            self.validated_data, ["issue", "fields", "reporter", "emailAddress"]
        )
        if reporter_email:
    ...
    def get_broker_submission_id_field_value(self):
        return self._data_get(
            self.initial_data, ["issue", "fields", "customfield_10303"]
        )

    def get_embargo_date_field_value(self):
        return self._data_get(
            self.initial_data, ["issue", "fields", "customfield_10200"]
        )
    ...
    def schema_validation(self, data):
        ...
        valid, errors = validate_data(
            data=data, schema_file=path, use_draft04_validator=True
        )
        if not valid:
            self.send_mail_to_admins(
                reason="Submission update via jira hook failed",
    ...
    def brokeragent_validation(self):
        ...
        logger.info(
            msg="serializers.py | brokeragent_validation | user {0}".format(
                updating_user
            )
        )
    ...
    def reporter_email_validation(self):
        repo_mail = self._data_get(
            self.initial_data, ["issue", "fields", "reporter", "emailAddress"]
        )

        # check for empty reporter mail:
        if not len(repo_mail):
            logger.info(
                msg="serializers.py | reporter_email_validation | reporter's Jira emailAddress is empty!"
            )
            self.send_mail_to_admins(
    ...
    def curator_validation(self):
        ...
        logger.info(
            msg="serializers.py | curator_validation | updating user {0}".format(
                updating_user
            )
        )
        # get curators
        ...
        if len(curators) == 0:
            ...
            raise serializers.ValidationError(
                {"issue": ["'user': user is not in curators group, no curators found"]}
            )

        curators_emails = [curator.email for curator in curators]
        if updating_user not in curators_emails:
            logger.info(
                msg="serializers.py | curator_validation | user {0} is not a crurator".format(
                    updating_user
                )
            )
            self.send_mail_to_admins(
            ...
            for s in studies:
                ...
                if not has_primary_accession and primary_accession is not None:
                    ...
                    self.send_mail_to_admins(
                        ...
                        " for submission without a primary accession (i.e. BioProject ID). Submission target {0}".format(
                            submission.target
                        ),
                    )
                    raise serializers.ValidationError(
                ...
                allowed = s.persistentidentifier_set.filter(
                    archive="ENA", pid_type="PRJ"
                ).filter(Q(status="PRIVATE") | Q(status="SUPPRESSED"))
                if allowed:
                    change_allowed = True
                    break
