...
class TicketLabel(models.Model):
    ...
    site_configuration = models.ForeignKey(SiteConfiguration, null=False, on_delete=models.PROTECT)
    label_type = models.CharField(max_length=1, choices=LABEL_TYPES)
    label = models.CharField(max_length=256, default="")

    def __str__(self):
        return "{0}_{1}_{2}".format(self.site_configuration.title, self.label_type, self.pk)
