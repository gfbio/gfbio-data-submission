...
class SiteConfiguration(TimeStampedModel):
    ...
    jira_project_key = models.CharField(
        choices=JIRA_PROJECT_KEYS, max_length=4, default=SAND
    )

    comment = models.TextField(
        default="", help_text="Enter a description or helpful text here."
    )
    ...
    def get_ticket_labels(self, label_type=""):
        return [
            label.label for label in self.ticketlabel_set.filter(label_type=label_type)
        ]
